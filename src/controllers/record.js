...
exports.insert = (db, record) => {
    return new Promise((resolve, reject) => {
      db.serialize(() => {
          db.run(querys.insert(TABLE, KEYS), 
          [record._transaction, record._date, record._quantity, record._product._id],
          (err) => {
...
exports.update = (db, record) => {
    return new Promise((resolve, reject) => {
        db.get(querys.update(TABLE, KEYS), [record._transaction, record._date, record._quantity, record._product._id, record._id], (err, row) => {
            if(err) reject(err.message)
            resolve(row)
        })
    })
